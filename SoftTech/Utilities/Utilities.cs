...
namespace SoftTech.Utilities
{
    public static class Utilities
    {
        ...
        {
            string secure_characters = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!@#$%&*";

            // Usar un generador de números aleatorios seguro
            Random random = new Random();

            // Generar una contraseña aleatoria combinando caracteres seguros
            var password = new StringBuilder(length);
            for (int i = 0; i < length; i++)
            {
                int index_character = random.Next(secure_characters.Length);
                password.Append(secure_characters[index_character]);
            }

            return password.ToString();
        }

        public static void SendEmail(string destinationEmail, string subject, string message)
        {
            try
            {
